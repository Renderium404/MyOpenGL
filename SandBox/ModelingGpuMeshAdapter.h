...
#include "Resource/ExternalGpuGeometryDataSource.h"

/// ModelingGpuMesh GPU Storage 到 MyOpenGL ExternalGpuGeometryDataSource 的 Adapter。
/// Adapter 只暴露外部 Buffer ID、GPU View Structure Revision 和 GPU Synchronization，不拥有 GPU Buffer。
class ModelingGpuMeshAdapter : public ExternalGpuGeometryDataSource
{
public:
    ...
    bool gpuView(ExternalGpuGeometryView& view) const override;
    ExternalGpuGeometryRevision structureRevision() const override;

    /// GPU View 同步
    bool prepareGpuViewGL(QOpenGLFunctions_3_3_Core* gl) const override; // MyOpenGL 获取新 GPU View 前等待 Worker Write Fence。
    void acknowledgeStructureRevision(ExternalGpuGeometryRevision revision) const override; // VAO 成功切换后记录 Renderer 已采用的 GPU Structure Revision。
